...
def spotlightExtractor(sourceFolder, username):
    try:
        ...
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if 'com.apple.spotlight.Shortcuts' in file:
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/spotlight')
    except:
    ...
    try:
        ...
        if os.path.isdir(fullpath) == True:
            for (path, dir, files) in os.walk(fullpath):
                for file in files:
                    if 'journalAttr.' in file:
                        bf.fileCopy(path + '/' + file, 'extractedFiles/spotlight')
    except:
...
def msofficeExtractor(sourceFolder, username):
    try:
        ...
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if 'com.microsoft.Word.plist' in file:
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/msoffice')
    except:
...
def googledriveExtractor(sourceFolder, username):
    try:
            ...
            for (path, dir, files) in os.walk(fullpath):
                for file in files:
                    if 'metadata_sqlite' in file or 'psid' in file or 'structured_log' in file or 'finder_ext' in file or 'driver_ext' in file:
                            bf.fileCopy(path+'/'+file, 'extractedFiles/googledrive')
    except:
        print("error - Google Drive")

def diagnosticExtractor(sourceFolder, username):
    try:
        fullpath = sourceFolder+'/private/var/db/diagnostics/Persist'
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if 'tracev3' in file:
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/diagonositc')
    except:
...
def recentfileExtractor(sourceFolder, username):
    try:
        ...
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if 'com.apple.textedit' in file or 'com.apple.preview' in file:
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/recentfiles')
    except:
...
def documentrevisionExtractor(sourceFolder, username):
    try:
        fullpath = sourceFolder+'/_.DocumentRevisions-V100/db-V1'
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if 'db.sqlite' in file:
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/documentrevision')
    except:
...
def printerExtractor(sourceFolder, username):
    try:
        fullpath = sourceFolder+'/private/var/spool/cups'
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if 'c0001' in file or 'd0001' in file:
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/printer')
    except:
        print("error - printer (permission denided)")

def webExtractor(sourceFolder, username):
    try:
        fullpath = sourceFolder+'/Users/' + username + '/Library/Preferences'
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if 'QuarantineEventsV2' in file:
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/web')
    except:
        print("error - web:QuarantineEventsV2")

def dsstoreExtractor(sourceFolder, username):
    #try:
    for folder in bf.checkFileInFolder(sourceFolder+'/Users/'+username):
        if folder == '.DS_Store':
            os.mkdir('extractedFiles/dsstore/'+'root')
            bf.fileCopy(sourceFolder+'/Users/' + username + '/' + folder, 'extractedFiles/dsstore/root')
        elif folder[0] == '.' or folder[0] == '_' or folder == 'Library':
            continue
        else:
            for file in bf.checkFileInFolder(sourceFolder+'/Users/'+username+'/'+folder):
                if 'DS_Store' in file:
                    os.mkdir('extractedFiles/dsstore/' + folder)
                    bf.fileCopy(sourceFolder+'/Users/'+username+'/'+folder+'/'+file, 'extractedFiles/dsstore/'+folder)
    #except:
        #print("error - dsstore")

def fseventsdExtractor(sourceFolder, username):
    try:
        fullpath = sourceFolder+'/_.fseventsd'
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                bf.fileCopy(fullpath + '/' + file, 'extractedFiles/fseventsd')
    except:
        print("error - fseventsd")

def knowledgeCExtractor(sourceFolder, username):
    try:
        fullpath = sourceFolder+'/private/var/db/CoreDuet/Knowledge'
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if 'knowledgeC' in file:
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/knowledgec')
    except:
        print("error - knowledgec (Permission denided)")

def notesExtractor(sourceFolder, username):
    try:
        ...
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if 'NoteStore.sqlite' in file or 'NoteStore.sqlite-wal' in file:
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/notes')
    except:
        print("error - notes")

def icloudExtractor(sourceFolder, username):
    try:
        fullpath = sourceFolder+'/Users/' + username + '/Library/Application Support/CloudDocs/session/db'
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if 'client' in file or 'server' in file:
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/icloud')
    except:
        print("Event - icloud")

def teamsExtractor(sourceFolder, username):
    try:
        fullpath = sourceFolder+'/Users/' + username + '/Library/Application Support/Microsoft/Teams/Cache'
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if file[-2:] == '_0':
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/teams')
    except:
...
def ichatExtractor(sourceFolder, username):
    try:
        fullpath = sourceFolder+'/Users/' + username + '/Library/Messages'
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if 'chat.db' in file:
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/ichat')
    except:
        print("error - chat.db")
    ...
    try:
        ...
        if os.path.isdir(fullpath) == True:
            for (path, dir, files) in os.walk(fullpath):
                if '.ichat' in files:
                    bf.fileCopy(path + '/' + files, 'extractedFiles/ichat')
    except:
        print("error - .ichat")

def safariExtractor(sourceFolder, username):
    try:
        ...
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if 'Downloads' in file or 'History.db' in file or 'RecentlyClosedTabs' in file:
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/safari')
    except:
    ...
    for (path, dir, files) in os.walk(fullpath):
        for file in files:
            if '-blob' in file:
                bf.fileCopy(path.replace('\\','/') + '/' + file, 'extractedFiles/safari')
    except:
...
def mailExtractor(sourceFolder, username):
    '''
    try:
        ...
        if os.path.isdir(fullpath) == True:
            for file in bf.checkFileInFolder(fullpath):
                if file == '1.qdat':
                    bf.fileCopy(fullpath + '/' + file, 'extractedFiles/mail')
    except:
            ...
            for (path, dir, files) in os.walk(fullpath):
                for file in files:
                    if 'Envelope Index' in file:
                        bf.fileCopy(path + '/' + file, 'extractedFiles/mail')
    except:
        print("error - mail:envelop")

def gmailExtractor(sourceFolder, username):
    try:
        fullpath = sourceFolder+'/Users/' + username + '/Library/Mail'
        if os.path.isdir(fullpath) == True:
            for (path, dir, files) in os.walk(fullpath):
                for file in files:
                    if 'partial.emlx' in file:
                        bf.fileCopy(path + '/' + file, 'extractedFiles/gmail')
    except:
        print("error - gmail")